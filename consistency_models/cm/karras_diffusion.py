...
from cm.custom_lpips import CustomLPIPS
...
class KarrasDenoiser:
    def __init__(
        ...
        if "lpips" in self.loss_norm:
            if self.args.lpips_model_path == "default":
                self.lpips_loss = LPIPS(replace_pooling=True, reduction="none")
            else:
                self.lpips_loss = CustomLPIPS(replace_pooling=True, reduction="none", model_path=args.lpips_model_path)
        self.rho = rho
        self.num_timesteps = args.diffusion_steps
        self.gan_training = args.gan_training
    ...
    def training_losses(self, model, x_start, sigmas, model_kwargs=None, noise=None):
        ...
        return terms
    def training_losses_with_determistic(self, model, x_start, sigmas, model_kwargs=None, noise=None, pre_pred=None):
        if model_kwargs is None:
            model_kwargs = {}
        else:
            model_kwargs = {"y":model_kwargs}
        if noise is None:
            noise = th.randn_like(pre_pred)
        terms = {}

        dims = pre_pred.ndim
        x_t = pre_pred + noise * append_dims(sigmas, dims) 
    ...
    def get_GAN_loss(self, model, real=None, fake=None, consistency_loss=None,
        ...
        if learn_generator:
            # Output obtained by passing through a discriminative network
    ...
    def consistency_losses(
        ...
        if self.loss_norm == "l1":
        ...
        elif self.loss_norm == "lpips_plus_l2":
            if x_start.shape[-1] < 256:
                distiller = F.interpolate(distiller, size=224, mode="bilinear")
                distiller_target = F.interpolate(
                    distiller_target, size=224, mode="bilinear"
            ...
            diffs = (distiller - distiller_target) ** 2
            l2_loss = mean_flat(diffs)
            loss = (lpips_loss + l2_loss) * weights
        else:
        ...
        if self.gan_training:
            ...
            gan_fake = distiller
            terms['gan'] = self.get_GAN_loss(model, fake=gan_fake,
                                                            consistency_loss=terms["cm_loss"],
                                                            discriminator=discriminator,
                                                )

        if "gan" in terms:
            terms["loss"] = terms["cm_loss"] + terms["gan"]
        else:
            terms["loss"] = terms["cm_loss"]
    ...
    def denoise(self, model, x_t, sigmas, **model_kwargs):
        ...
        rescaled_t = 1000 * 0.25 * th.log(sigmas + 1e-44) # t to input model 
        if self.task == "bsr":
            model_output = model(c_in * x_t, rescaled_t, **model_kwargs)
        else:
            model_output = model(c_in * x_t, rescaled_t, s=rescaled_t, **model_kwargs)
        denoised = c_out * model_output[:,:x_t.shape[1]] + c_skip * x_t
        return model_output, denoised
class KarrasDenoiserbsr(KarrasDenoiser):
    def __init__(
        self,
        sigma_data: float = 0.5,
        sigma_max=80.0,
...
def karras_sample_bsr(
    ...
    if sampler == "multistep_bsr_v2":
        x_T = generator.randn(*shape, device=device) * sigma_max + pre_pred
    else:
    ...
    sample_fn = {
        ...
        "multistep_bsr": stochastic_iterative_sampler_bsr,
        "multistep_bsr_v2": stochastic_iterative_sampler_bsr_v2,
    }[sampler]
    if sampler in ["heun", "dpm"]:
        sampler_args = dict(
            s_churn=s_churn, s_tmin=s_tmin, s_tmax=s_tmax, s_noise=s_noise
        )
    elif sampler == "multistep" or sampler == "multistep_bsr" or sampler == "multistep_bsr_v2":
        sampler_args = dict(
            ts=ts, t_min=sigma_min, t_max=sigma_max, rho=diffusion.rho, steps=steps, time_calculator=time_calculator
        )
    else:
        sampler_args = {}
    ...
    if time_calculator is not None:
        ...
        return x_0.clamp(-1, 1), elapsed_avg_t
    x_0 = sample_fn(
        denoiser,
        x_T,
        sigmas,
...
def sample_onestep(
 ...
 # sampler for multistep diffusion
@th.no_grad()
...
@th.no_grad()
def stochastic_iterative_sampler_bsr(
    distiller,
    x,
    sigmas,
    generator,
    ...
    t_max_rho = t_max ** (1 / rho) #when t_max=80, t_max_rho=0.14285714285714285
    t_min_rho = t_min ** (1 / rho) #when t_min=0.002, t_min_rho=0.41155971378360795
    s_in = x.new_ones([x.shape[0]])
    times = []
    for i in range(len(ts) - 1):
        t = (t_max_rho + ts[i] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho #calculate current t's noise level (when i=0, t=t_max_rho, when i=steps-1, t=t_min_rho) 
        if i==0:
            # x0 = pre_pred + x
            x0 = pre_pred
        else:
            if time_calculator is not None:
                ...
                x0 = distiller(x, t * s_in) # denoise x with current t
        next_t = (t_max_rho + ts[i + 1] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho #calculate next t's noise level
        breakpoint()
        next_t = np.clip(next_t, t_min, t_max) # kept within the range
        if ts[i]!=steps-1:
            x = x0 + generator.randn_like(x) * np.sqrt(next_t**2 - t_min**2)
        else:
            x = x0
        ...
        # if callback is not None:
        #     callback({'x0':x0,'x': x, 'i': i, 't': t, 'data_info': data_info, 'pred_model_name': pred_model_name,}) 
    if time_calculator is not None:
        return x, sum(times)
    return x

@th.no_grad()
def stochastic_iterative_sampler_bsr_v2(
    distiller,
    x,
    sigmas,
    generator,
...
def save_image_callback(data):
    """Callback function to save intermediate images during sampling."""
    # Assuming data is a dictionary containing 'x' (the image) and 'i' (the step)
    image = data['x']
    step = data['t']
    image_x0 = data['x0']
    order = data['i']
    pred_model_name = data['pred_model_name']
    meta_info = data['data_info'][0]
    data_info = meta_info['burst_name']
    process_fn = SimplePostProcess(return_np=True)
    image = ((image + 1) / 2).clamp(0.0, 1.0)
    image_x0 = ((image_x0 + 1) / 2).clamp(0.0, 1.0)
    image_bgr = image.clone()
    image_x0_bgr = image_x0.clone()
    image_bgr[:,0,...], image_bgr[:,2,...] = image[:,2,...], image[:,0,...]
    image_x0_bgr[:,0,...], image_x0_bgr[:,2,...] = image_x0[:,2,...], image_x0[:,0,...]
    image_bgr = process_fn(image_bgr[0], meta_info)
    image_x0_bgr = process_fn(image_x0_bgr[0], meta_info)

    # Convert image from [-1, 1] range to [0, 1] range and save
    torchvision.utils.save_image(image_x0_bgr, f"{logger.get_dir()}/{data_info}_intermediate_image_x0_{pred_model_name}_{order}th_{step}.png")
    torchvision.utils.save_image(image_bgr, f"{logger.get_dir()}/{data_info}_intermediate_image_{pred_model_name}_{order}th_{step}.png")
