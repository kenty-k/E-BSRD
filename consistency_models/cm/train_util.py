...
class TrainLoop:
    ...
    def log_step(self):
        ...
        wandb.log({"samples": (self.step + self.resume_step + 1) * self.global_batch})
        ...
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                if not rate:
                    ...
                    with bf.BlobFile(bf.join(get_blob_logdir(self.save_dir), filename), "wb") as f:
                        th.save(state_dict, f)
                # else:
                #     filename = f"ema_{rate}_{(self.step+self.resume_step):06d}.pt"
                # with bf.BlobFile(bf.join(get_blob_logdir(self.formattednow), filename), "wb") as f:
                #     th.save(state_dict, f)
...
class CMTrainLoop_burst(TrainLoop):
    def __init__(
        self,
        *,
        target_model,
    ...
    def forward_backward(self,frame_gt, burst):
        ...
        for i in range(0, frame_gt.shape[0], self.microbatch):
            micro = frame_gt[i : i + self.microbatch].to(dist_util.dev())
    ...
    def save(self):
        ...
        def save_checkpoint(rate, params,dir_name):
            state_dict = self.mp_trainer.master_params_to_state_dict(params)
            if dist.get_rank() == 0:
                logger.log(f"saving model {rate}...")
                if not rate:
                    filename = f"model{step:06d}.pt"
                    with bf.BlobFile(bf.join(get_blob_logdir(dir_name), filename), "wb") as f:
                        th.save(state_dict, f)
                # else:
        ...
        for rate, params in zip(self.ema_rate, self.ema_params):
            save_checkpoint(rate, params,dir_name=self.dir_name)
        ...
        if dist.get_rank() == 0:
            if self.target_model:
                ...
                with bf.BlobFile(bf.join(get_blob_logdir(self.dir_name), filename), "wb") as f:
        ...
        # Save model parameters last to prevent race conditions where a restart
        # loads model at step N, but opt/ema state isn't saved for step N.
        save_checkpoint(0, self.mp_trainer.master_params,dir_name=self.dir_name)
        dist.barrier()
    ...
    def log_step(self):
        ...
        wandb.log({"step": step})
        wandb.log({"samples": (step + 1) * self.global_batch})
...
def get_blob_logdir(dir_name="edm_scratch"):
    # You can change this to be a separate path to save checkpoints to
    # a blobstore or some external drive.
        dir = "./logs/"+str(dir_name)
        return dir
...
class TrainLoopBSR_EDM(TrainLoop):
    def forward_backward(self, frame_gt, burst):
        wandb_batch_loss = {}
        self.mp_trainer.zero_grad()
        for i in range(0, frame_gt.shape[0], self.microbatch):
            ...
            compute_losses = functools.partial( # self, model, x_start, sigmas, model_kwargs=None, noise=None)
                self.diffusion.training_losses,
                self.ddp_model,
                micro_gt,
                t,
    ...
    def run_loop(self):
        ...
        if (self.step - 1) % self.save_interval != 0:
            self.save()
class TrainLoopBSR_EDM_with_Determistic(TrainLoopBSR_EDM):
    def __init__(
        self,
        *,
        model,
        ...
        dataset_type = "synthetic",
        save_dir=None,
        determistic_model=None,
        train_determistic = False

    ):
        self.dir_name = save_dir
        self.determistic_model = determistic_model
        self.train_determistic = train_determistic
        self.model = model
        self.diffusion = diffusion
        self.data = data
        self.batch_size = batch_size
        ...
        self.step = self.resume_step
        self.save_dir = save_dir
        self.determistic_model = self.determistic_model
    ...
    def forward_backward(self, frame_gt, burst):
        ...
        for i in range(0, frame_gt.shape[0], self.microbatch):
            ...
            ############### pre model ##################
            pre_preds = []
            for i in range(micro_burst.shape[0]):
                if self.train_determistic:
                    pre_pred = self.determistic_model(micro_burst[i].unsqueeze(0))
                else:
                    with th.no_grad():
                        pre_pred = self.determistic_model(micro_burst[i].unsqueeze(0))
                pre_preds.append(pre_pred.squeeze(0))
            pre_preds = th.stack(pre_preds, dim=0)
            compute_losses = functools.partial( # self, model, x_start, sigmas, model_kwargs=None, noise=None)
                self.diffusion.training_losses_with_determistic,
                self.ddp_model,
                micro_gt,
                t,
                model_kwargs=micro_burst,
    ...
    def save(self):
        ...
        for rate, params in zip(self.ema_rate, self.ema_params):
            save_checkpoint(rate, params, dir_name=self.dir_name)
        # Save the deterministic model using PyTorch Lightning's format
        if dist.get_rank() == 0:
            logger.log(f"saving deterministic model in PyTorch Lightning format...")
            determistic_model_path = bf.join(get_blob_logdir(self.dir_name), f"determistic_model{(self.step+self.resume_step):06d}.ckpt")
            th.save(
                {
                    'state_dict': self.determistic_model.state_dict(),
                    'step': self.step + self.resume_step,
                    'optimizer_states': self.opt.state_dict(),
                },
                determistic_model_path
            )
            logger.log(f"Deterministic model saved at {determistic_model_path}")
