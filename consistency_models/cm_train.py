...
def main():
    ...
    wandb.init(project="consistency models", name = "original model without flash atten"+str(formatted_now), config=vars(args))  # プロジェクト名と設定を指定
    ...
    ema_scale_fn = create_ema_and_scales_fn( # モデルの指数移動平均(EMA)やスケールの設定
    ...
    model, diffusion = create_model_and_diffusion(**model_and_diffusion_kwargs) # 拡散モデルのインスタンスを作成
...
def create_argparser(): # コマンドラインから引数を受け取るための関数
    defaults = dict(
        data_dir="",
