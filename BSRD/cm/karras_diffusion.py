...
class KarrasDenoiser:
    def __init__(
        ...
        sigma_max=80.0, # change
        sigma_min=0.002,
        rho=7.0,
        weight_schedule="karras",
        distillation=False,
        loss_norm="lpips", #try mse
        num_steps=40,
    ):
...
def karras_sample_bsr(
    ...
    if sampler == "progdist":
        ...
        sigmas = get_sigmas_karras(steps, sigma_min, sigma_max, rho, device=device)# change sigma max,steps
    if pre_pred is None:
        x_T = generator.randn(*shape, device=device) * sigma_max
    else:
        x_T = pre_pred*1 + generator.randn(*shape, device=device) * sigma_max  # change( 0→pre_pred), add pre_pred
        import os
        import matplotlib.pyplot as plt
        from data_processing.postprocessing_functions import SimplePostProcess
        process_fn = SimplePostProcess(return_np=True)
        save_dir = f"./xT_saved_images/{sigma_max}"
        os.makedirs(save_dir, exist_ok=True)
        print("x_T.shape =", x_T.shape)
        print("x_T.mean  =", x_T.mean().item())
        print("x_T.std   =", x_T.std().item())
        print("x_T.min   =", x_T.min().item())
        print("x_T.max   =", x_T.max().item())

        x_T_cpu = x_T.detach().cpu()  # GPU -> CPU
        batch_size = x_T_cpu.shape[0]
        ...
        for i in range(batch_size):
            ...
            filename = os.path.join(save_dir, f"xT_{i:04d}.png")
            plt.imsave(filename, img.numpy())  
            print(f"Saved {filename}")

    sample_fn = {
        "heun": sample_heun,
        "heun_bsr": sample_heun_bsr,
        "dpm": sample_dpm,
        ...
        "DBSR": stochastic_iterative_sampler_bsr,
    }[sampler]
    if sampler in ["heun", "dpm","heun_bsr"]:
        sampler_args = dict(
            s_churn=s_churn, s_tmin=s_tmin, s_tmax=s_tmax, s_noise=s_noise
        )
    elif sampler == "multistep" or sampler == "multistep_bsr":
        sampler_args = dict(
...
def sample_heun_bsr(
    ...
    for i in indices:
        gamma = ( # calculate how often noise to add
            min(s_churn / (len(sigmas) - 1), 2**0.5 - 1)
            if s_tmin <= sigmas[i] <= s_tmax
            else 0.0
        )
