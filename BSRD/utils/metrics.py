...
class LPIPS(nn.Module):
    def __init__(self, boundary_ignore=None, type='alex', bgr2rgb=False, model_path=None):
        super().__init__()
        self.boundary_ignore = boundary_ignore
        self.bgr2rgb = bgr2rgb
        if model_path is not None:
            if type == 'alex':
                self.loss = lpips.LPIPS(net='alex', model_path=model_path)
            elif type == 'vgg':
                self.loss = lpips.LPIPS(net='vgg', model_path=model_path)
            else:
                raise Exception
        else:
            if type == 'alex':
